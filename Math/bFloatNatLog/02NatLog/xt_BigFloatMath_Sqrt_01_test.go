...
func Test_sqrtBigFloat_1000Digits_03(t *testing.T) {

  ePrefix := "Test_sqrtBigFloat_1000Digits_03()"
  ...
  _, xValFracDigits := bFloatHlpr.CountDigits(xValStr, '.')

  expectedResultStr = bFloatHlpr.CleanNumberString(expectedResultStr)

  _, expectedResultDecimalDigits := bFloatHlpr.CountDigits(expectedResultStr, '.')

  calculationDecimalDigits := uint(1001 + 5)

  precBits := bFloatHlpr.ComputeBigFloatPrecisionBits(calculationDecimalDigits, 1)
  ...
  actual, err := sqrtCalc.SqrtBigFloatDigits(xVal, calculationDecimalDigits)
  ...
  if err != nil {
    t.Errorf("%v\n"+
      ...
      "actual, err := sqrtCalc.SqrtBigFloatDigits(xVal, calculationDecimalDigits)\n"+
      "xVal= '%v'\n"+
      "calculationDecimalDigits= '%v'\n"+
      "Error= '%v'\n\n",
      ePrefix, xVal.Text('f', xValFracDigits), calculationDecimalDigits, err.Error())
    return
  }

  actualStr := actual.Text('f', expectedResultDecimalDigits)
