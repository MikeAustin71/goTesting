...
// areAstronomicalYearsAdjacent - Compares the year values of
// input parameters 'astronomicalYear1', and 'astronomicalYear2'
// to determine if the two year values are adjacent. Adjacent
// years are defined here as differing by a value of plus or
// minus one year. If the year values are 'Adjacent', the difference
// as calculated by 'astronomicalYear1 - astronomicalYear2' will
// always equal to plus or minus 1 (+1 or -1).
//
// The 'Adjacent Year' comparison implemented by calculating
// the value of 'astronomicalYear1 - astronomicalYear2' will
// always have one of three possible outcomes.
//
//  1. If 'astronomicalYear1' IS ADJACENT to 'astronomicalYear2',
//     and the difference as calculated by 'astronomicalYear1 -
//     'astronomicalYear2' is plus one (+1), it signals that
//     'astronomicalYear1' is GREATER than 'astronomicalYear2'
...
//
//  2. If 'astronomicalYear1' IS NOT ADJACENT to 'astronomicalYear2',
//     the calculation result of 'astronomicalYear1 - astronomicalYear2'
...
//     be set to zero ('0').
//
//  3. If 'astronomicalYear1' IS ADJACENT to 'astronomicalYear2',
//     and the difference as calculated by 'astronomicalYear1 -
//     'astronomicalYear2' is minus one (-1), it signals that
//     'astronomicalYear1' is LESS THAN 'astronomicalYear2' by a
...
//
// Remember that both input parameters 'astronomicalYear1' and
// 'astronomicalYear2' must be formatted according to the Astronomical
// Year Numbering System.
//
// The Astronomical Year Numbering System includes year zero. Under this
// year numbering system, the date January 1, year 1 is immediately
// preceded by the date December 31, year 0. Years prior to year zero (0)
// are negative values prefixed with a minus sign. For example, years
// before year zero (0) are numbered as -1, -2, -3, -4 etc.
//
...
// Input Parameters
//
//  astronomicalYear1        int64
//     - A year number expressed as an int64 value. This year value
//       will be compared to input parameter, 'astronomicalYear2' in
//       order to determine whether the two years are adjacent years
//       as defined in the narrative above.
//
//      'astronomicalYear1' should be formatted using the Astronomical
//       Year Numbering System. The Astronomical Year Numbering System
//       includes year zero. Under this year numbering system, the date
//       January 1, year 1 is immediately preceded by the date December
//       31, year 0. Years prior to year zero (0) are negative values
//       prefixed with a minus sign. For example, years before year zero
//       (0) are numbered as -1, -2, -3, -4 etc.
//
//       As its name implies, Astronomical Year Numbering is frequently
//       used in astronomical calculations. For additional information on
//       the Astronomical Year Numbering System, reference:
...
//
//
//  astronomicalYear2        int64
//     - A year number expressed as an int64 value. This year value
//       will be compared to input parameter, 'astronomicalYear1' in
//       order to determine whether the two years are adjacent years
//       as defined in the narrative above.
//
//      'astronomicalYear2' should be formatted using the Astronomical
//       Year Numbering System. The Astronomical Year Numbering System
//       includes year zero. Under this year numbering system, the date
//       January 1, year 1 is immediately preceded by the date December
//       31, year 0. Years prior to year zero (0) are negative values
...
//     - This boolean return value will signal whether the input parameters
//       'astronomicalYear1' and 'astronomicalYear2' are adjacent years.
//       Adjacent years differ by a value of plus or minus 1-year.
//
//       If the 'astronomicalYear1' and 'astronomicalYear2' are adjacent
...
//       obtained from determining whether input parameters
//       'astronomicalYear1' and 'astronomicalYear2' are adjacent years.
//       As such, it will be populated with one of three possible values.
//
//       1. If 'astronomicalYear1' IS ADJACENT to 'astronomicalYear2',
//          and the difference as calculated by 'astronomicalYear1 -
//          astronomicalYear2' is plus one (+1), it signals that
//          'astronomicalYear1' is GREATER than 'astronomicalYear2'
//          by a value of '+1'. For this case, the returned integer
//          value of 'compareResult' is set to +1.
//
//       2. If 'astronomicalYear1' IS NOT ADJACENT to 'astronomicalYear2',
//          the calculation result of 'astronomicalYear1 - astronomicalYear2'
...
//          value of 'compareResult' is set to -1.
//
func (calMech *calendarMechanics) areAstronomicalYearsAdjacent(
	astronomicalYear1 int64,
	astronomicalYear2 int64) (
	areYearsAdjacent bool,
	compareResult int) {
	...
	variance := astronomicalYear1 - astronomicalYear2

	if variance == -1 {
		compareResult = -1
		areYearsAdjacent = true
		return areYearsAdjacent, compareResult
	}
...
// convertAnyYearToAstronomicalYear - This method receives a year value
// and a CalendarYearNumType instance. The CalendarYearNumType
// argument is used to classify the 'year' value as Astronomical
// Year type, Before Common Era Year Type or Common Era Year Type.
//
// After evaluating the year type code, the method then proceeds to
// convert 'year' to an Astronomical Year Value which is returned to
...
//
// In the 'Common Era Year Numbering system the date January 1, year 1 is
// labeled as 'CE' or 'C.E.' designating year 1 as the first year of the
// Common Era. January 1, year 1 is immediately preceded by the date
// December 31, year 1 BCE. The Common Era Year Numbering system does not
// include or recognize a year zero (0). The date December 31, year 0 in
// the Astronomical Year Numbering System is formatted as December 31, year 1 BCE
// in the Common Era Year Numbering system. Note that 'BCE' or 'B.C.E' means
// 'Before Common Era'.
//
// The Common Era begins at midnight on January 1, year 1. Dates prior to this
// date are labeled as 'Before Common Era' or BCE.
//
//
// The input parameter 'yearNumType' is of type 'CalendarYearNumType' and
// allows the calling function to designate one of three types of years:
//
...
// Return Values
//
//  astronomicalYearValue  int64
//     - If this method completes successfully, input parameter, 'year',
//       will converted to an Astronomical Year Value and returned in
...
//
//
func (calMech *calendarMechanics) convertAnyYearToAstronomicalYear(
	year int64,
	yearNumType CalendarYearNumType,
	ePrefix string) (
	astronomicalYearValue int64,
	...
	ePrefix += "calendarMechanics.convertAnyYearToAstronomicalYear() "

	astronomicalYearValue = math.MinInt64
	err = nil

	switch yearNumType {

		case CalendarYearNumType(0).Astronomical() :
			astronomicalYearValue = year
			return astronomicalYearValue, err

		case CalendarYearNumType(0).CE() :
			if year <= 0 {
				err = fmt.Errorf(ePrefix + "\n" +
					"Invalid input parameter 'yearNumType'!\n" +
					"'yearNumType' and 'year' are conflicted.\n" +
					"'yearNumType'= 'Common Era' and 'year' is less than '1'.\n" +
					"yearNumType='%v'\n" +
					"year='%v'\n",
					yearNumType.String(),year)
				return astronomicalYearValue, err
			} else {
				astronomicalYearValue = year
				return astronomicalYearValue, err
		...
		case CalendarYearNumType(0).BCE() :
			if year <= 0 {
				err = fmt.Errorf(ePrefix + "\n" +
					"Invalid input parameter 'yearNumType'!\n" +
					"'yearNumType' and 'year' are conflicted.\n" +
					"'yearNumType'= 'Before Common Era' and 'year' is less than '1'.\n" +
					"yearNumType='%v'\n" +
					"year='%v'\n",
					yearNumType.String(),year)
				return astronomicalYearValue, err
				...
				astronomicalYearValue = (year + 1) * -1
				return astronomicalYearValue, err
			}
...
// convertAstronomicalYearToCommonEraYear - This method is designed
// to convert a year value formatted with Astronomical Year Numbering
// to a  year value formatted with Common Era Year Numbering.
//
// Astronomical Year Numbering System
//
// The Astronomical Year Numbering System includes a year zero (0). In
...
// Years occurring before year zero are preceded with a minus sign
// (January 1, -1). To illustrate, years before year one (1) are
// numbered like this: 1, 0, -1, -2, -3, -4, -5 etc. For more information
// on the the 'Astronomical Year Numbering System', reference:
//      https://en.wikipedia.org/wiki/Astronomical_year_numbering
//
// Common Era Year Numbering System
...
// date are labeled as 'Before Common Era' or BCE.
//
// To recap, the Common Era Numbering System supports to year types:
//
//       1. Before Common Era Numbering
//            All years preceding the date January 1, year 1. These years typically
//            have the suffix BCE, for Before Common Era. For example, the date
//            immediately preceding January 1, 0001 CE is December 31, 0001 BCE.
//            year preceding year 0001 BCE are numbered as 2 BCE, 3 BCE, 4 BCE etc.
//
//       2. Common Era Numbering
//            All years following, or greater than, the date December 31, 1 BCE. These
//            years often have the suffix CE signaling they are years in the Common Era.
//            For example years following the date December 31, 1 BCE are numbered 1 CE,
//            2 CE, 3 CE, 4 CE etc.
...
//     - A year value. This year value is assumed to be formatted using the
//       Astronomical Year Numbering System. If this year value is not
//       formatted using the Astronomical Year Numbering System, this method
...
// Return Values
//
//  commonEraYearValue    int64
//     - If this method completes successfully the input parameter will be
//       converted to its Common Era equivalent and returned in this parameter.
//       Given the definition of year values formatted with the Common Era
//       Numbering System, this return value will always be a positive number
//       greater than or equal to +1.
//
//
//  commonEraYearNumType  CalendarYearNumType
//     - A Year Number Type enumeration. This 'CalendarYearNumType' enumeration
//       will always be set to one of two values:
//
//       1. Before Common Era Numbering (BCE) = CalendarYearNumType(0).BCE()
//            All years preceding the date January 1, year 1. These years typically
//            have the suffix BCE, for Before Common Era. For example, the date
//            immediately preceding January 1, 0001 CE is December 31, 0001 BCE.
//            year preceding year 0001 BCE are numbered as 2 BCE, 3 BCE, 4 BCE etc.
//
//       2. Common Era Numbering (CE) = CalendarYearNumType(0).CE()
//            All years following, or greater than, the date December 31, 1 BCE. These
//            years often have the suffix CE signaling they are years in the Common Era.
//            For example years following the date December 31, 1 BCE are numbered 1 CE,
//            2 CE, 3 CE, 4 CE etc.
//
//
func (calMech *calendarMechanics) convertAstronomicalYearToCommonEraYear(
	astronomicalYear int64) (
	commonEraYearValue int64,
	commonEraYearNumType CalendarYearNumType) {
	...
	commonEraYearValue = math.MinInt64
	commonEraYearNumType = CalYearType.None()

	if astronomicalYear < 1 {
		commonEraYearValue = astronomicalYear * -1
		commonEraYearValue += 1
		commonEraYearNumType = CalYearType.BCE()

		return commonEraYearValue, commonEraYearNumType
	}

	commonEraYearValue = astronomicalYear
	commonEraYearNumType = CalYearType.CE()

	return commonEraYearValue, commonEraYearNumType
}

// getCalendarYearByType - Returns the correct Calendar Year
// Type plus a correctly formatted year number.
//
//
// ------------------------------------------------------------------------
//
// Input Parameters
//
//  astronomicalYearNo       int64
//     - A year number styled using the Astronomical Year Numbering
//       System. This year numbering system includes a year zero (0).
//       In other words, the date January 1, year 1 is immediately
//       preceded by the date December 31, year zero (0). Likewise,
//       the date January 1, year 0 is immediately preceded by the
//       date December 31, year -1.
//
//       As shown, years occurring before year zero are preceded with
//       a minus sign (January 1, -1). This year numbering system is
//       the default numbering system used by the datetime software
//       library.
//
//       For more information on the 'Astronomical Year Numbering
//       System', reference:
//            https://en.wikipedia.org/wiki/Astronomical_year_numbering
//
//
...
//      which will be returned by this method.  The choices are:
...
//       'yearNumType' described below.
//
//
//  actualYearNumType        CalendarYearNumType
//     - This enumeration value designates the returned year value
//       described above (actualYearNum) as one of three year types:
//         1. Astronomical Year
...
//
//
func (calMech *calendarMechanics) getCalendarYearByType(
	astronomicalYearNo int64,
	targetYearNumberingMode CalendarYearNumMode,
	ePrefix string) (
	actualYearNum int64,
	...
	ePrefix += "calendarMechanics.getCalendarYearByType() "

	actualYearNumType = CalendarYearNumType(0).None()
	actualYearNum = astronomicalYearNo
	err = nil

	if targetYearNumberingMode == CalYearMode.Astronomical() {
		actualYearNumType = CalYearType.Astronomical()
		return actualYearNum, actualYearNumType, err
	}

	if targetYearNumberingMode == CalYearMode.CommonEra() {

		if astronomicalYearNo < 0 {

			actualYearNum = astronomicalYearNo * -1
			actualYearNum += 1
			actualYearNumType = CalYearType.BCE()

		} else if astronomicalYearNo == 0 {
			...
			actualYearNum = astronomicalYearNo
			actualYearNumType = CalYearType.CE()
		}
	...
	err = fmt.Errorf(ePrefix + "\n" +
		"targetYearNumberingMode is INVALID!\n" +
		"targetYearNumberingMode='%v'\n",
		targetYearNumberingMode.XValueInt())
...
// Warning:
//
// This method cannot process negative Julian Day Numbers
//
// ------------------------------------------------------------------------
//
// Input Parameters
...
func (calMech *calendarMechanics) julianDayNoTimeToGregorianCalendar(
	...
	bigJulianDayNo, err2 = julianDayNoDto.GetJulianDayBigInt(ePrefix)
...
func (calMech *calendarMechanics) revisedGoucherParkerToJulianDayNo(
	...
	isCurrentLeapYear := calRevGoucherParkerMech.isLeapYear(years)
...
//
//   Mike's Notes: Programming Local - Conversions - Calendars Richards
//       15.11.2 Page 618
//
// For more information on the Julian Day Number see:
//   https://en.wikipedia.org/wiki/Julian_day
//
// ------------------------------------------------------------------------
//
...
//      to a day of the week number. For more information on the
//      Julian Day Number see:
//           https://en.wikipedia.org/wiki/Julian_day
//
//        type JulianDayNoDto struct {
//          julianDayNo         *big.Int   // Julian Day Number expressed as integer value
...
func (calMech *calendarMechanics) usDayOfWeekNumber(
	...
	jDN, err2 := julianDayNumber.GetJulianDayBigInt(ePrefix)
