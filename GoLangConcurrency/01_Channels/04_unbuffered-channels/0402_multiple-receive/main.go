...
func search(msg chan<- MessageDto, done chan bool, wg *sync.WaitGroup, threshold int, target int) {
	...
	for i := threshold; i < max; i++ {
		...
		select {
		case <-done:
			dto.CountMsg = fmt.Sprintf(" From Done Threshold: %v  Count: %v", threshold, dto.SearchCount)
			msg <- dto
			wg.Done()
			return
		default:
	...
	for {
		select {
		case <-done:
			dto.CountMsg = fmt.Sprintf("From Final Exit Threshold: %v  Count: %v", threshold, dto.SearchCount)
			msg <- dto
			wg.Done()
			return
		default:
