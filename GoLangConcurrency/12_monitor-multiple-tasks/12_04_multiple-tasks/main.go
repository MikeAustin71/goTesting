...
func search(msg chan<- MessageDto, done chan bool, wg *sync.WaitGroup, threshold int, target int) {
	...
	for i := threshold; i < max; i++ {
		...
		select {
			case <-done:
				dto.CountMsg = fmt.Sprintf("Done - Threshold: %v  Count: %v", threshold, dto.SearchCount)
				msg <- dto
				wg.Done()
				return
			default:
				if i == target {
					...
					dto.IsSuccess = true
					dto.CountMsg = fmt.Sprintf("Target Loc - Threshold: %v  Count: %v", threshold, dto.SearchCount)
					msg <- dto
					done <- true
					close(done)
					wg.Done()
